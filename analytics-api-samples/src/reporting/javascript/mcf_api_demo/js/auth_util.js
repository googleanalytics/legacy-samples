...
function handleAuthResult(authResult) {
  document.getElementById('button-pane').style.visibility = '';
  if (authResult) {
    // v3alpha is for MCF API
    gapi.client.load('analytics', 'v3alpha', handleAuthorized);
  } else {
    handleUnauthorized();
  }
}
 ...
 * makeMcfApiCall click handler to the authorize-and-run-button.
 */
function handleAuthorized() {
  var button = document.getElementById('authorize-and-run-button');
  button.innerHTML = 'Run Demo';
  button.style.visibility = '';
  button.onclick = makeMcfApiCall;
  out.set('Click the Run Demo button to begin.');
}
 ...
 * click handler to the authorize-and-run-button.
 */
function handleUnauthorized() {
  ...
  out.set('Please authorize this script to access Google Analytics.');
}
